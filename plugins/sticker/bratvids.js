...
let handler = async (m, { conn, text }) => {
  if (!text || !text.trim()) throw 'text entrer non valid!';
  ...
  try {
    ...
    if (!res.ok) throw `echec de l' API! Status: ${res.status}`;
    ...
    await conn.sendMessage(m.chat, { text: `Error: ${err.message || 'error création brat.'}` }, { quoted: m });
  }
};
