...
let handler = async (m, { conn, usedPrefix, text }) => {
    try {
        ...
        if (!mime || !mime.startsWith('image/')) throw `Send/Reply image with caption ${usedPrefix}removebg`;
        ...
        let apiUrl = `https://api.ryzendesu.vip/api/ai/v2/removebg?url=${url}&upscale=${upscale}`;
        let response = await fetch(apiUrl);
        if (!response.ok) throw new Error('erreur de l API');

        let hasil = await response.buffer();

        await conn.sendFile( m.chat, hasil, 'removedbg.jpg', 'Background removed successfully!', m );
