    ...
    try {
        ...
        for (let name in global.plugins) {
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                if(xp > 200)
                    // m.reply('Cheating -_-') // Hehehe
                    console.log("cheating -_-");
                else
                    m.exp += xp
                if(!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
                    this.reply(m.chat, `[❗] Limit reached, please contact for premium subscription *${usedPrefix}premium*`, m)
                    continue // Limit reached
                }
                if(plugin.level > _user.level) {
                    this.reply(m.chat, `[💬] Level ${plugin.level} required to use this command\n*Your level:* ${_user.level} 📊`, m)
                ...
                try {
                    ...
                    if(m.limit)
                        m.reply(+m.limit + ' Your limit has been used')
                }
                break
            }
        }
...
To delete messages sent by Bot, reply to the message with the command
*.delete*`, msg)
...
global.dfail = (type, m, conn) => {
    let msg = {
        rowner: '*ONLY DEVELOPER* • THIS COMMAND IS ONLY FOR BOT DEVELOPER',
        owner: '*ONLY OWNER* • THIS COMMAND IS ONLY FOR BOT OWNER',
        mods: '*ONLY MODERATOR* • THIS COMMAND IS ONLY FOR BOT MODERATOR',
        ...
        botAdmin: '*ONLY BOT ADMIN* • THIS COMMAND CAN ONLY BE USED WHEN BOT IS AN ADMIN',
        unreg: '*YOU ARE NOT REGISTERED YET* • TYPE .register TO USE THIS FEATURE',
        restrict: '*RESTRICT* • RESTRICT HAS NOT BEEN ENABLED IN THIS CHAT',
        disable: '*DISABLED* • THIS CMD HAS BEEN DISABLED BY OWNER', 
    }[type]
